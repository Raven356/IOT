...
class config(old_config):
    ...
    def _compile(self, body, headers, include_dirs, lang):
        src, obj = self._wrap_method(
            old_config._compile, lang, (body, headers, include_dirs, lang)
        )
    ...
    def check_header(self, header, include_dirs=None, library_dirs=None, lang="c"):
        ...
        return self.try_compile(
            "/* we need a dummy line to make distutils happy */", [header], include_dirs
        )
    ...
    def check_type_size(
        ...
        while True:
            try:
                self._compile(
                    body % {"type": type_name, "size": mid}, headers, include_dirs, "c"
                )
                self._clean()
                break
            except CompileError:
